...
var bckground_music, waterSound, pirateLaughSound,CanonExplosion;
...
function preload() {
  ...
  bckground_music = loadSound("assets/background_music.mp3");
...
function draw() {
  ...
  if(! bckground_music.isPlaying()) {
    bckground_music.play();
    bckground_music.setVolume(0.1);
  }
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        isGameOver = true;
        if(! is_laughing && ! pirateLaughSound.isPlaying()) {
          is_laughing = true;
